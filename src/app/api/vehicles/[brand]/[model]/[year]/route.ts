 ...
 * Returns all listings for a brand/model/year from different sellers.
...
interface Params { brand: string; model: string; year: string }
...
export async function GET(_req: NextRequest, { params }: { params: Params }) {
  ...
  if (!brand || !model || isNaN(year)) {
    return NextResponse.json({ ok: false, error: 'brand, model, and year are required' }, { status: 400 })
  }
  ...
  try {
    const { data: products, error: pErr } = await db
      .from('products')
      .select(`id, name, slug, image_url, vehicle_brand, vehicle_model, vehicle_year, vehicle_type,
    ...
    if (pErr) {
      const { data: fallback } = await db.from('products').select('id, name, slug, image_url').ilike('name', `%${brand}%${model}%${year}%`).limit(20)
      return NextResponse.json({ ok: true, listings: fallback ?? [], source: 'name-fallback', year, brand, model })
    }

    if (!products || products.length === 0) return NextResponse.json({ ok: true, listings: [], year, brand, model })

    const ids = products.map((p: { id: string }) => p.id)
    const { data: offers } = await db
      .from('offers')
      .select(`product_id, price, url, affiliate_url, location, condition, merchants ( name, slug, color, logo_url )`)
      .in('product_id', ids)
      .eq('in_stock', true)
    ...
    const listings = products.map((p: {
      ...
      vehicle_type: string | null; vehicle_mileage: number | null; vehicle_transmission: string | null;
      vehicle_color: string | null; vehicle_location: string | null;
    }) => ({
      ...p,
      offers: (offersByProduct[p.id] ?? []).sort((a: { price: number }, b: { price: number }) => a.price - b.price),
    ...
    return NextResponse.json({ ok: true, brand: params.brand, model: params.model, year, total: listings.length, listings })
  } catch (err) {
