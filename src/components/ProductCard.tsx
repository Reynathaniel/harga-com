...
import { TrendingDown, ShoppingCart, Bell, Globe, Flame } from 'lucide-react'
...
export function ProductCard({ product, compact = false, tilt = true }: ProductCardProps) {
  ...
  return (
    <Link href={'/produk/' + product.id} className="block" style={{ transformStyle: 'preserve-3d' }}>
      <div ref={cardRef}
        ...
        style={{
          ...
          boxShadow: hovered ? '0 8px 28px rgba(26,22,19,0.10), 0 2px 8px rgba(212,146,10,0.08)' : 'var(--shadow-card)',
        ...
        <div className="relative overflow-hidden" style={{ aspectRatio: '1 / 1', background: 'var(--cream-100)' }}>
          <MediaEmbed
          ...
          <div className="absolute flex flex-col gap-1.5 items-start" style={{ top: 10, left: 10, transform: 'translateZ(40px)' }}>
            {diff > 0 && (
          ...
          <div className="absolute bottom-0 left-0 right-0 px-2 pt-8 pb-2" style={{
            background: 'linear-gradient(to top, rgba(26,22,19,0.75), rgba(26,22,19,0.3), transparent)',
            opacity: hovered ? 1 : 0, transition: 'opacity var(--transition-base)',
          }}>
            <div className="flex items-center justify-between">
              <span style={{ fontSize: 'var(--text-10)', fontWeight: 'var(--fw-bold)', color: 'var(--brand)' }}>
        ...
        <div style={{ padding: compact ? '10px' : '12px', display: 'flex', flexDirection: 'column', gap: 8, flex: 1, transform: 'translateZ(24px)' }}>
          <h3 style={{
            margin: 0, fontFamily: 'var(--font-ui)',
            fontSize: compact ? 'var(--text-xs)' : 'var(--text-sm)',
            fontWeight: 'var(--fw-medium)', lineHeight: 'var(--leading-snug)',
          ...
          {!compact && domesticListings.length > 0 && intlListings.length > 0 && (
            <div className="flex items-center gap-0.5" style={{ fontSize: 'var(--text-9)', color: 'var(--info)', fontWeight: 'var(--fw-semibold)', opacity: 0.8 }}>
          ...
          <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: 6 }}>
            <div>
              ...
              <div className="flex items-center gap-1 mt-1">
                ...
                <span style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)' }}>di {cheapestPlatform.shortName}</span>
              </div>
            </div>

            {savings > 0 && (
              <span className="inline-flex items-center gap-1" style={{
                fontFamily: 'var(--font-sans)', fontSize: 'var(--text-11)', fontWeight: 'var(--fw-bold)',
                color: 'var(--win)', background: 'var(--win-soft-bg)',
                border: '1px solid var(--win-soft-border)', padding: '3px 8px',
          ...
          {!compact && rangePercent > 3 && (
            <div>
              <div className="h-1 rounded-full overflow-hidden" style={{ background: 'var(--cream-200)' }}>
                <div className="h-full rounded-full" style={{ background: 'var(--gradient-win)', width: `${Math.max(10, 100 - rangePercent)}%` }} />
              </div>
          ...
          {!compact && cashbackPct > 0 && (
            <span className="inline-flex items-center gap-1" style={{
              ...
              border: '1px solid var(--brand-soft-border)', alignSelf: 'flex-start',
            }}>
              CB {cashbackPct}% · {formatRupiah(cashbackAmount, true)}
            </span>
          )}
            ...
            {displayListings.map(l => {
              ...
              return (
                <div key={l.platformId}
                  ...
                  style={{ background: l.platformId === 'tiktok' ? '#1a1a1a' : p.color, fontSize: 9, fontWeight: 800 }}
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              style={{ width: 32, height: 32, border: '1px solid var(--border-subtle)', background: 'transparent', cursor: 'pointer', color: 'var(--text-muted)' }}
              onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'var(--brand-soft-border)'; e.currentTarget.style.background = 'var(--brand-soft-bg)'; e.currentTarget.style.color = 'var(--brand)' }}
              onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--border-subtle)'; e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--text-muted)' }}
